...
//
// NOTE: All vars are  because of issues on android. Will investigate and resolve at later date.
 ...
 float fbm( vec2 pos,  float T)
{
 ...
 vec3 clouds(  vec2 vUv ,  vec3 skycol,  float blend,  float T ) {
	 vec2 p = vUv;
	 float coverage = fparams.y;

	 float w = fbm(p*1.+T+.05, T);
	w=min(max(0.,w-coverage)*(1./coverage) ,1.);

	 float v = 1.*fbm(p*1.+T, T);
...
void main() {
	...
	// in scattering
	 ...
	 vec3 betaT = ( betaRTheta + betaMTheta ) / ( vBetaR + vBetaM );
	 vec3 Lin = pow( vSunE * betaT * ( 1.0 - Fex ),  vec3( 1.5 ) );
	 float mixVal = clamp( pow( 1.0 - dot( up, vSunDirection ), 5.0 ), 0.0, 1.0 );
	Lin *= mix(  vec3( 1.0 ), pow( vSunE * betaT * Fex,  vec3( 0.5 ) ), mixVal );
	...
	L0 += ( vSunE * 190000.0 * Fex ) * sundisk;
	 ...
	 vec3 curr 		= Uncharted2Tonemap(  texColor * fparams.w );
	...
	// Use world object to cal projected sphere to planar
	 ...
	 vec2 pos 		=  vec2(vCloudPosition.x * 0.002, vCloudPosition.z * 0.002);
	...
	if (vCloudPosition.y > cloudHeight ) {
		...
		retColor = clouds( pos, retColor, ( vCloudPosition.y - test )/blendheight , T); 
	}
	gl_FragColor =  vec4( retColor * vSunfade * 1.15, 1.0 );
}

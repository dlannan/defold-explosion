...
vec4 raymarch( in vec3 rayo, in vec3 rayd, in float expInter, in vec2 fragCoord )
{
    ...
    vec3 pos = rayo + rayd * (expInter + step*texture( iChannel0, fragCoord.xy/256 ).x);
...
void main()
{
    ...
    vec2 q = var_texcoord0.xy; // / iResolution.xy;
    vec2 p = -1.0 + 2.0*q;
    ...
    {
        // yes, cast ray
        col = raymarch( cameraPos, rayDir, boundingSphereInter, var_texcoord0 );
    }

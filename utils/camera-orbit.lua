...
local function defaulthandler( self, delta )
	...
	if (yaw > limits.yaw[1]) then yaw = yaw-2.0*math.pi; self.yangle = yaw end 
	if (yaw < limits.pitch[2]) then yaw = yaw+2.0*math.pi; self.yangle = yaw end 
	if (pitch > limits.pitch[1]) then pitch = limits.pitch[1]; self.xangle = limits.pitch[1] end 
	if (pitch < limits.pitch[2]) then pitch = limits.pitch[2]; self.xangle = limits.pitch[2] end 
	...
	self.tpos 		= go.get_world_position(self.target)
